    ...
    def step(self, robot_action, drone_action):
        ...
        if not done and self.robot == self.goal:
            ...
            if (NUM_OBSTACLES == len(self.seen_obstacles)) and (robot_action == 8):
                ...
                robot_reward -= (self.MOVE_PENALTY * math.exp((NUM_OBSTACLES - len(
                    self.seen_obstacles) + 1) * self.MOVE_PENALTY_EXP_CONSTANT * self.episode_step))
            ...
            if robot_to_goal <= self.GOAL_PROXIMITY_THRESHOLD:
                robot_reward += (self.GOAL_PROXIMITY_MAX_REWARD * math.exp(
                    self.GOAL_REWARD_EXP_CONSTANT * (self.SIZE - robot_to_goal)))
        ...
        if len(self.seen_obstacles) != NUM_OBSTACLES:
            drone_reward -= self.DRONE_MOVE_DISCOUNT * (
                        self.MOVE_PENALTY * math.exp(self.MOVE_PENALTY_EXP_CONSTANT * self.episode_step))
...
if TRAINING:
    ...
    np.savetxt(f'results/model' + MODEL_NUMBER + '_multi/model' + MODEL_NUMBER + '_multi_training_rewards_unified.csv',
               ep_rewards)
...
if not TRAINING:
    ...
    if not os.path.isdir('results/model' + MODEL_NUMBER + '_multi'):
        os.makedirs('results/model' + MODEL_NUMBER + '_multi')
    np.savetxt(f'results/model' + MODEL_NUMBER + '_multi/model' + MODEL_NUMBER + '_multi_testing_rewards.csv',
               ep_rewards)
