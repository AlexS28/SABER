...
class BlobEnv:
    ...
    def step(self, robot_action, drone_action):
        ...
        return new_observation, robot_reward, drone_reward, done
...
if TRAINING:
    ...
    for episode in tqdm(range(1, EPISODES + 1), ascii=True, unit='episodes'):
        ...
        drone_agent.tensorboard.step = episode

        # Restarting episode - reset episode reward and step number
        episode_robot_reward = 0
        episode_drone_reward = 0
        step = 1
        ...
        while not done:
            if np.random.random() > robot_epsilon:
                ...
                robot_action = np.argmax(robot_agent.get_qs(current_state))
            else:
                # Get random action
                robot_action = np.random.randint(0, env.ACTION_SPACE_SIZE)

            if np.random.random() > drone_epsilon:
                #Get action from Q table
                drone_action = np.argmax(drone_agent.get_qs(current_state))
            else:
                # Get random action
                drone_action = np.random.randint(0, env.ACTION_SPACE_SIZE)

            new_state, robot_reward, drone_reward, done = env.step(robot_action, drone_action)
        ...
        if not episode % AGGREGATE_STATS_EVERY or episode == 1:
            ...
            # Save model
            robot_agent.model.save(f'models/model' + MODEL_NUMBER + '_multi_robot.model')
    ...
    np.savetxt(f'results/model' + MODEL_NUMBER + '_multi/model' + MODEL_NUMBER + '_multi_training_rewards_robot.csv', ep_rewards)
